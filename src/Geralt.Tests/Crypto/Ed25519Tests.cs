...
public class Ed25519Tests
{
    ...
    // https://github.com/C2SP/wycheproof/blob/main/testvectors_v1/ed25519_test.json
    public static IEnumerable<object[]> WycheproofTestVectors()
    {
        ...
        // Can't use CompressedSignature, SignatureWithGarbage, or TruncatedSignature due to length validation
    }
    ...
    {
        yield return [Ed25519.SignatureSize + 1, 1, Ed25519.PrivateKeySize];
        yield return [Ed25519.SignatureSize - 1, 1, Ed25519.PrivateKeySize];
        yield return [Ed25519.SignatureSize, 1, Ed25519.PrivateKeySize + 1];
        yield return [Ed25519.SignatureSize, 1, Ed25519.PrivateKeySize - 1];
    }

    public static IEnumerable<object[]> VerifyInvalidParameterSizes()
    {
        yield return [Ed25519.SignatureSize + 1, 1, Ed25519.PublicKeySize];
        yield return [Ed25519.SignatureSize - 1, 1, Ed25519.PublicKeySize];
        yield return [Ed25519.SignatureSize, 1, Ed25519.PublicKeySize + 1];
        yield return [Ed25519.SignatureSize, 1, Ed25519.PublicKeySize - 1];
    }
