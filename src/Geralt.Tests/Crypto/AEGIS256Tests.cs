...
public class AEGIS256Tests
{
    ...
    public static IEnumerable<object[]> InvalidParameterSizes()
    {
        ...
        yield return [AEGIS256.TagSize, 0, AEGIS256.NonceSize - 1, AEGIS256.KeySize, AEGIS256.TagSize];
        yield return [AEGIS256.TagSize, 0, AEGIS256.NonceSize, AEGIS256.KeySize + 1, AEGIS256.TagSize];
        yield return [AEGIS256.TagSize, 0, AEGIS256.NonceSize, AEGIS256.KeySize - 1, AEGIS256.TagSize];
    }
    ...
    public void Encrypt_Invalid(int ciphertextSize, int plaintextSize, int nonceSize, int keySize, int associatedDataSize)
    {
        ...
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => AEGIS256.Encrypt(c, p, n, k, ad));
    }
    ...
    public void Decrypt_Tampered(string ciphertext, string plaintext, string nonce, string key, string associatedData)
    {
        ...
        foreach (var param in parameters.Values.Where(param => param.Length > 0)) {
            param[0]++;
            Assert.ThrowsExactly<CryptographicException>(() => AEGIS256.Decrypt(p, parameters["c"], parameters["n"], parameters["k"], parameters["ad"]));
            param[0]--;
        }
    ...
    public void Decrypt_Invalid(int ciphertextSize, int plaintextSize, int nonceSize, int keySize, int associatedDataSize)
    {
        ...
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => AEGIS256.Decrypt(p, c, n, k, ad));
    }
}
