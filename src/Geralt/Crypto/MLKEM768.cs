...
{
    public const int PublicKeySize = crypto_kem_mlkem768_PUBLICKEYBYTES;
    public const int PrivateKeySize = crypto_kem_mlkem768_SECRETKEYBYTES;
    public const int SeedSize = crypto_kem_mlkem768_SEEDBYTES;
    public const int SharedSecretSize = crypto_kem_mlkem768_SHAREDSECRETBYTES;
    public const int CiphertextSize = crypto_kem_mlkem768_CIPHERTEXTBYTES;
    ...
    public static void GenerateKeyPair(Span<byte> publicKey, Span<byte> privateKey)
    {
        ...
        Sodium.Initialize();
        int ret = crypto_kem_mlkem768_keypair(publicKey, privateKey);
    ...
    public static void GenerateKeyPair(Span<byte> publicKey, Span<byte> privateKey, ReadOnlySpan<byte> seed)
    {
        ...
        Sodium.Initialize();
        int ret = crypto_kem_mlkem768_seed_keypair(publicKey, privateKey, seed);
    ...
    public static void Encapsulate(Span<byte> sharedSecret, Span<byte> ciphertext, ReadOnlySpan<byte> recipientPublicKey)
    {
        ...
        Sodium.Initialize();
        int ret = crypto_kem_mlkem768_enc(ciphertext, sharedSecret, recipientPublicKey);
    ...
    public static void Decapsulate(Span<byte> sharedSecret, ReadOnlySpan<byte> ciphertext, ReadOnlySpan<byte> recipientPrivateKey)
    {
        ...
        Sodium.Initialize();
        int ret = crypto_kem_mlkem768_dec(sharedSecret, ciphertext, recipientPrivateKey);
        if (ret != 0) { throw new CryptographicException("Error performing decapsulation."); }
    }
}
