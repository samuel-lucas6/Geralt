...
public static class ChaCha20
{
    ...
    {
        Validation.EqualTo($"{nameof(ciphertext)}.{nameof(ciphertext.Length)}", ciphertext.Length, plaintext.Length);
        Validation.EqualTo($"{nameof(nonce)}.{nameof(nonce.Length)}", nonce.Length, NonceSize);
        Validation.EqualTo($"{nameof(key)}.{nameof(key.Length)}", key.Length, KeySize);
    ...
    {
        Validation.EqualTo($"{nameof(plaintext)}.{nameof(plaintext.Length)}", plaintext.Length, ciphertext.Length);
        Validation.EqualTo($"{nameof(nonce)}.{nameof(nonce.Length)}", nonce.Length, NonceSize);
        Validation.EqualTo($"{nameof(key)}.{nameof(key.Length)}", key.Length, KeySize);
