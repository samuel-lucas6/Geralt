...
{
    public const int KeySize = crypto_aead_xchacha20poly1305_ietf_KEYBYTES;
    public const int NonceSize = crypto_aead_xchacha20poly1305_ietf_NPUBBYTES;
    ...
    public static void Encrypt(Span<byte> ciphertext, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> nonce, ReadOnlySpan<byte> key, ReadOnlySpan<byte> associatedData = default)
    {
        ...
        Sodium.Initialize();
        int ret = crypto_aead_xchacha20poly1305_ietf_encrypt(ciphertext, ciphertextLength: out _, plaintext, (ulong)plaintext.Length, associatedData, (ulong)associatedData.Length, nsec: null, nonce, key);
    ...
    public static void Decrypt(Span<byte> plaintext, ReadOnlySpan<byte> ciphertext, ReadOnlySpan<byte> nonce, ReadOnlySpan<byte> key, ReadOnlySpan<byte> associatedData = default)
    {
        ...
        Validation.EqualToSize(nameof(key), key.Length, KeySize);
        Sodium.Initialize();
        int ret = crypto_aead_xchacha20poly1305_ietf_decrypt(plaintext, plaintextLength: out _, nsec: null, ciphertext, (ulong)ciphertext.Length, associatedData, (ulong)associatedData.Length, nonce, key);
